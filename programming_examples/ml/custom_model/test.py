...
def main(opts):
    ...
    pad = (width + 31) // 32 * 32 - width
    widtho = width + pad
    shape_out = (height, co8, widtho, 8)
    ...
    # ------------------------------------------------------
    temp_out = data_buffer.reshape(height, co8, widtho, 8)
    temp_out = ds.reorder_mat(temp_out, "CDYX", "YCXD")
    #print("**********************"*100)
    #print(temp_out.shape)
    #from IPython import embed
    #embed()
    ofm_mem_fmt = temp_out.reshape(co, widtho, height)
    ...
    embed()
    if pad != 0:
        ofm_mem_fmt_out = ofm_mem_fmt_out[:,:,:-pad,:].permute(0,1,3,2) # Convert to NCHW and remove padding
    
    # print statistics of ofm_mem_fmt
    print("ofm_mem_fmt statistics:")
    print("  min: {}".format(ofm_mem_fmt_out.min()))
    print("  max: {}".format(ofm_mem_fmt_out.max()))
    print("  mean: {}".format(ofm_mem_fmt_out.mean()))
    print("  std: {}".format(ofm_mem_fmt_out.std()))
    print("  shape: {}".format(ofm_mem_fmt_out.shape))
    # print statistics of golden_output
    print("golden_output statistics:")
    print("  min: {}".format(golden_output.min()))
    print("  max: {}".format(golden_output.max()))
    print("  mean: {}".format(golden_output.mean()))
    print("  std: {}".format(golden_output.std()))
    print("  shape: {}".format(golden_output.shape))
    if np.allclose(
